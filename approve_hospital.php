  <?php
      ...
      $db=new dbconnect();
          $var_id=$_GET['mvarid'];
		   $varsql="select email from hospital  where id='$var_id'";
	 $var=mysql_query($varsql);
	 $row=mysql_fetch_array($var);
	 $var_email=$row['email'];
	  $pass=rand(100,1000);
     
	  $upd="insert into login(username,password,userlevel)values('$var_email','$pass','3')";
      $ans_upd=mysql_query($upd) or die(mysql_error());
       $varsql="update  hospital set status=1 where id='$var_id'";
