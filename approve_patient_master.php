  ...
  error_reporting(0);
      ...
      $db=new dbconnect();
          $var_id=$_GET['mvarid'];
		  $varsql="select email from patient_master where id='$var_id'";
	 $var=mysql_query($varsql);
	 $row=mysql_fetch_array($var);
	 $var_email=$row['email'];
	  $pass=rand(100,1000);
	    $upd="insert into login(username,password,userlevel)values('$var_email','$pass','4')";
      $ans_upd=mysql_query($upd)  or die(mysql_error());
	  
	   $varsql="UPDATE patient_master SET status=1 WHERE  id='$var_id'";
...
<td></td>
       ...
       $pm->Send(); 
     echo '<script>window.location = "Patient_master_Select.php?pp=4";</script>';
  ?>
